...
from TaigaRobot.modules import ALL_MODULES
from TaigaRobot.modules.helper_funcs.chat_status import is_user_admin
from TaigaRobot.modules.helper_funcs.misc import paginate_modules
...
DONATE_STRING = """Heya, glad to hear you want to donate!
 You can support the project by adding bot to ypur group \
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("TaigaRobot.modules." + module_name)
...
def start(update: Update, context: CallbackContext):
    ...
    if update.effective_chat.type == "private":
        if len(args) >= 1:
            ...
            update.effective_message.reply_photo(photo=PM_MEDIA,
        ...
        update.effective_message.reply_video(video=G_MEDIA,
...
def Taiga_about_callback(update, context):
    query = update.callback_query
    chat = update.effective_chat
    if query.data == "Taiga_":
        query.message.edit_caption(
            caption=gs(chat.id, "pm_about_text"),
            parse_mode=ParseMode.MARKDOWN,
            #disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(text="ᴀᴅᴍɪɴs", callback_data="Taiga_admin"),
                        #InlineKeyboardButton(text=gs(chat.id, "notes_button"), callback_data="Taiga_notes"),
                    ],
                    [
                        #InlineKeyboardButton(text=gs(chat.id, "support_chat_link_button"), callback_data="Taiga_support"),
                        # InlineKeyboardButton(text=gs(chat.id, "darling_button"), url="tg://settings"),
                        InlineKeyboardButton(text="ᴄʀᴇᴅɪᴛs", callback_data="Taiga_credit"),
                    ],
                    [
                    InlineKeyboardButton(text=gs(chat.id, "back_button"), callback_data="Taiga_back"),
                    ]
                ]
            ),
        )
    elif query.data == "Taiga_back":
        first_name = update.effective_user.first_name
        uptime = get_readable_time((time.time() - StartTime))
        query.message.edit_caption(
                caption=gs(chat.id, "pm_start_text").format(
    ...
    elif query.data == "Taiga_admin":
        query.message.edit_caption(
            caption=gs(chat.id, "pm_about_admin_text"),
            parse_mode=ParseMode.MARKDOWN,
            #disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
                [
    ...
    elif query.data == "Taiga_notes":
        query.message.edit_caption(
            caption=gs(chat.id, "pm_about_notes_text"),
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup(
                [
                    [
    ...
    elif query.data == "Taiga_support":
        query.message.edit_caption(
            caption=gs(chat.id, "pm_about_support_text"),
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup(
                [
                    [
    ...
    elif query.data == "Taiga_credit":
        query.message.edit_caption(
            caption=gs(chat.id, "pm_about_credit_text"),
            parse_mode=ParseMode.MARKDOWN,
            #disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
                [
...
def main():
    ...
    if SUPPORT_CHAT is not None and isinstance(SUPPORT_CHAT, str):
        try:
            dispatcher.bot.send_video(f"@{SUPPORT_CHAT}", video=ALIVE_MEDIA, caption=f"Im Alive again Darling", parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup(
                [
                  [
    ...
    about_callback_handler = CallbackQueryHandler(
        Taiga_about_callback, pattern=r"Taiga_", run_async=True
    )
